...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Customer Directory</h1>
        <p class="text-sm text-slate-500">Manage accounts, tiers, and subscription statuses.</p>
      </div>

      <button
        class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white shadow-xs transition-all hover:bg-blue-700 active:scale-95"
      >
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-xs">
      ...
      <div class="relative w-full sm:w-80">
        ...
        <input
          ...
          class="w-full rounded-xl border border-slate-200 bg-slate-50/50 pl-9 pr-4 py-2 text-xs font-medium text-slate-800 placeholder-slate-400 outline-hidden transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100"
        />
      </div>

      <!-- Plan Level Dropdown -->
      <div class="flex items-center gap-2 w-full sm:w-auto">
        <span class="text-xs font-medium text-slate-500 whitespace-nowrap">Plan Level:</span>
        <select
          v-model="selectedPlan"
          class="w-full sm:w-44 rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-semibold text-slate-700 outline-hidden transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100"
        >
    ...
    <TableData :columns="columns" :data="filteredCustomers">
      ...
      <template #cell-plan="{ value }">
        <span
          ...
          :class="{
            'bg-amber-50 text-amber-700 border border-amber-200': value === 'Enterprise',
            'bg-blue-50 text-blue-700 border border-blue-200': value === 'Professional',
            'bg-slate-100 text-slate-700 border border-slate-200': value === 'Starter',
            'bg-slate-50 text-slate-500 border border-dashed border-slate-300': value === 'Trial'
          }"
        >
      ...
      <template #cell-status="{ value }">
        <span
          ...
          :class="{
            'bg-emerald-50 text-emerald-600': value === 'Active',
            'bg-amber-50 text-amber-600': value === 'Pending',
      ...
      <template #cell-usagePercent="{ value }">
        <div class="w-28 space-y-1">
          <div class="flex justify-between text-[10px] font-bold text-slate-500">
            <span>Quota</span>
            <span :class="isHighUsage(value) ? 'text-amber-600 font-extrabold' : 'text-slate-600 dark:text-slate-400'">
              {{ formatPercent(value) }}
            </span>
          </div>
          <div class="h-1.5 w-full rounded-full bg-slate-100 overflow-hidden">
            <div
              ...
              :class="isHighUsage(value) ? 'bg-amber-500' : 'bg-blue-600'"
      ...
      <template #cell-action="{ row }">
        <button
          ...
          class="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-bold text-white shadow-xs transition-all hover:bg-blue-700 active:scale-95"
        >
          <span>View</span>
