...
const userChartData: ChartData<'line'> = {
  labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'],
  datasets: [
    {
      label: 'Active Users',
...
const billingChartData: ChartData<'doughnut'> = {
...
const revenueColumns: Column<RevenueRow>[] = [
  { key: 'period', label: 'Period', sortable: true, formatter: (val) => `$${Number(val).toLocaleString()}` },
  ...
  { key: 'growth', label: 'Growth Rate', sortable: true, formatter: (val) => `$${Number(val).toLocaleString()}` }
]
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Executive Reports</h1>
        <p class="text-sm text-slate-500">Cross-departmental performance metrics and financial summaries.</p>
      </div>

      <button
        @click="triggerExcelExport"
        class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white shadow-xs hover:bg-emerald-700 active:scale-95 transition-all"
      >
    ...
    <div class="relative rounded-2xl border border-slate-200 bg-white p-1.5 shadow-xs">
      ...
      <div class="relative flex overflow-x-auto no-scrollbar">
        ...
        <button
          ...
          class="flex items-center justify-center gap-2 whitespace-nowrap rounded-xl px-5 py-4 text-xs font-bold transition-colors z-10"
    ...
    <div v-if="activeTab === 'revenue'" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
          <h2 class="text-base font-bold text-slate-900 mb-1">Monthly Revenue Velocity</h2>
          <p class="text-xs text-slate-500 mb-6">Comparison between recurring subscriptions and add-on expansions.</p>
          <div class="h-72">
            <Bar :data="revenueChartData" :options="barChartOptions" />
          </div>
        </div>

        <div class="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
          <div>
            <h2 class="text-base font-bold text-slate-900 mb-2">Executive Summary</h2>
            <div class="my-4 border-t border-slate-100"></div>
            <div class="space-y-4 text-xs">
              <div>
                <span class="text-slate-400 block font-semibold">Total ARR</span>
                <p class="text-2xl font-extrabold text-slate-900">$816,000</p>
                <span class="text-[11px] font-bold text-emerald-600">▲ +18.4% vs last quarter</span>
              </div>
              <div>
                <span class="text-slate-400 block font-semibold">Average Revenue Per User (ARPU)</span>
                <p class="text-lg font-bold text-slate-800">$185.50 / mo</p>
              </div>
            </div>
          </div>

          <div class="mt-6 rounded-xl bg-emerald-50/50 p-3.5 border border-emerald-100 text-xs text-emerald-900">
    ...
    <div v-else-if="activeTab === 'users'" class="space-y-6">
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
        <h2 class="text-base font-bold text-slate-900 mb-1">User Growth & Conversion</h2>
        <p class="text-xs text-slate-500 mb-6">Tracking active accounts versus paying subscribers.</p>
        <div class="h-80">
    ...
    <div v-else-if="activeTab === 'billing'" class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
          <h2 class="text-base font-bold text-slate-900 mb-1">Invoice Status Distribution</h2>
          <p class="text-xs text-slate-500 mb-6">Breakdown of current billing collection efficiency.</p>
        ...
        <div class="space-y-4">
          <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex items-center justify-between">
            <div>
              <span class="text-xs text-slate-400 font-bold block">COLLECTED THIS MONTH</span>
              <span class="text-xl font-extrabold text-emerald-600">$68,400.00</span>
            </div>
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs flex items-center justify-between">
            <div>
              <span class="text-xs text-slate-400 font-bold block">PENDING INVOICES</span>
    ...
    <div v-else-if="activeTab === 'growth'" class="space-y-6">
      <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
        <h2 class="text-base font-bold text-slate-900 mb-1">Quarterly Growth Velocity (%)</h2>
        <p class="text-xs text-slate-500 mb-6">Quarter-over-quarter account expansion rate.</p>
        <div class="h-80">
